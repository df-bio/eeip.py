...
class Encapsulation:
    ...
    class CIPIdentityItem:
        ...
        def __init__(self):
            ...
            self.encapsulation_protocol_version = (
                int()
        ...
        def get_cip_identity_item(self, starting_byte, receive_data):
            ...
            self.encapsulation_protocol_version = receive_data[4 + starting_byte] | (
                receive_data[5 + starting_byte] << 8
            )
            ...
            self.socket_address.sin_address = (
                receive_data[13 + starting_byte]
                | (receive_data[12 + starting_byte] << 8)
                | (receive_data[11 + starting_byte] << 16)
                | (receive_data[10 + starting_byte] << 24)
            )
            ...
            self.serial_number = (
                receive_data[32 + starting_byte]
                | (receive_data[33 + starting_byte] << 8)
                | (receive_data[34 + starting_byte] << 16)
                | (receive_data[35 + starting_byte] << 24)
            )
